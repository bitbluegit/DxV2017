...
<div class="bg-white overflow-x box-shadow margin-bottom30  ">
    ...
    <form class="row" method="post" name="bonafide-filter" id="bonafide-filter" action="#" onsubmit="return false">
        ...
        <!-- Standard -->
        <div class="col m12 l4">
            <label for="year" class="font-weight100 small-caps full-width">Standard</label>
    ...
    <table class="full-width margin-bottom-zero">
        ...
        <tbody id="filter-data">
            ...
            $sql = " SELECT AD.`name` AS 'user_name' ,BF.`gr_no`, BF.`name` AS 'stu_name' ,
                    ...
                    BF.`sr_no` AS sr_no FROM clg_bonafide BF INNER JOIN admin_sch AD ON AD.`unique_id` = BF.`unique_id`";
           $data  = DB::allRow($sql);
if($data){
    foreach ($data as $row) {
          $sr_no = array_pop($row);
         ...
         $btn = "<button class='btn btn-green' onclick='printBonafide({$sr_no})'><i class='ion-ios-printer'></i>
          ...
          <button class='btn btn-red' onclick='deleteBonafide({$sr_no})'><i class='ion-trash-b'></i> </button>"; 
...
<script src="ReprintBonafide.js"></script>

<script type="text/javascript">
// reprint
   function printBonafide(srno){
                    var sr_no = srno;
                    // alert(sr_no);
                    window.location.href ="printBonafide.php?srno="+sr_no+""
...
// alert(srno);
        ...
        fn = function(){
         ...
         AjaxPost('deleteBonafide.php',params,fn,'txt');
