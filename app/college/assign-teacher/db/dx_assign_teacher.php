...
forEach($assignSubArr as $assignSub){
	$insertTeacherQuery .= " INSERT INTO `clg_exam_teacher` (`unique_id`, `exam_sub_id`, `teacher_id`, `date` )
...
if($result){
	...
	$updateIsAssignStatus = mysqli_query($con ," UPDATE clg_exam_subjects ES SET ES.`is_assign`= 1 WHERE ES.`exam_sub_id` IN ({$examSubIds}) " );
	...
	if($updateIsAssignStatus){
		...
		forEach($assignTeacherArr as $tId => $subNameArr){
			...
			$msgToClassQuery = "
			INSERT INTO clg_chat 
			(
			teacher_id, msg, date, msg_status, user_type, mdm, year, course, course_name, sec
		) 
		VALUES
		(
		'{$user_id}', '{$msg}', '{$currentDateTime}', '1', '1', '{$mdm}', '{$yr}', '{$cors}', '{$corsName}', '{$div}'
		) ; "
